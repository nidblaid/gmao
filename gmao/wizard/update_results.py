...
class UpdateResults(models.TransientModel):
    ...
    def _get_analytics_values(self, salesperson, data_type):
        ...
        worked_hours = self.env['hr.attendance'].search([('employee_id.user_id.id', '=', salesperson.id),('check_in', '>=', self.start_date), ('check_out', '<=', self.end_date)]).mapped('worked_hours')
        lead_count = self.env['crm.lead'].search_count(lead_domain)
        dispatched_count = self.env['crm.lead'].search_count([('user_id', '=', salesperson.id),('date_conversion', '>=', self.start_date), ('date_conversion', '<=', self.end_date), ('active', 'in', [True, False]),('data_status', '=', 'cold' if data_type == 'cold' else 'fresh')])
        processed_count = self.env['lead.processing.history'].search_count([('salesperson_id', '=', salesperson.id), ('date_processing', '>=', self.start_date), ('date_processing', '<=', self.end_date), ('data_status', '=', 'cold' if data_type == 'cold' else 'fresh')])
        ...
        worked_days = sum(worked_hours) * 0.73 / 8 if (data_type == 'fresh') else 0
        values = {
            'leads': lead_count,
