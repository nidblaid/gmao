...
class SaleOrder(models.Model):
    ...
    # General case : based on odoo base field
    ...
    def create(self, vals):
        ...
        sale_order._add_carrier_line()
        return sale_order

    def write(self, vals):
        res = super(SaleOrder, self).write(vals)
        for order in self:
            order._add_carrier_line()
        return res


    def _add_carrier_line(self):
        for order in self:
            if order.carrier_id:
                carrier_product = self.carrier_id.product_id
                if carrier_product:
                    line = order.order_line.filtered(lambda l: l.product_id.id == order.carrier_id.product_id.id)
                    if line:
                        line.write({
                            'product_uom_qty': 1,
                        })
                        ...
                        order.order_line.create({
                            ...
                            'name': order.carrier_id.product_id.name,
                            'product_uom_qty': 1,
                            'price_unit': 0,
                        })
                else:
                    raise UserError(f'There is no product realated to this carrier "{order.carrier_id.name}"')
